...
from Feature_CNN_Chakrabarty import Feature_CNN_Chakrabarty
...
class Dataset_Testing_CNN_Chakrabarty_random_rebuttal(Dataset):
    ...
    def __getitem__(self, index):
        ...
        self.mic_coordinates_array = 2 * (
                np.random.rand(self.num_channels, 3) - 0.5) * self.max_sensor_spread
        ...
        self.mic_coordinates_array -= np.mean(self.mic_coordinates_array, axis=0)
        ...
        x = self.generate_sample_CPU(room_dim=room_dim, rt_60_desired=rt_60_desired,
                                     ...
                                     array=coordinates.clone(), mic_center=mic_center)
        ...
        #                          2*self.max_difference_samples, self.device, False)

        # True coordinates known to gadoae
        feature = Feature_CNN_Chakrabarty(self.frame_length, self.num_channels, self.device)

        # chans = 5
        # axes = []
        # fig1 = plt.figure()
        # for a in range(chans):
        #     axes.append(fig1.add_subplot(1, chans, a + 1))
        #     plt.plot(x[a, :])
        #     plt.axis('off')
        # fig1.tight_layout()
        # plt.show()
        ...
        for frame in range(num_frames):
            ...
            m_out_feature[frame, :] = feature.calculate(x[:, idx_in:idx_out])

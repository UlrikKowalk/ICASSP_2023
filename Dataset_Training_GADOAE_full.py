...
class Dataset_Training_GADOAE_full(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # self.max_dist_array = 2 * self.calculate_mic_distance(self.mic_coordinates_array[0, :],
        #                                                       self.mic_coordinates_array[-1, :])

        # Needs to be fixed value due to DNN structure
        ...
        # Source signal
        # x = torch.zeros(size=(self.num_channels, int(self.sample_rate * self.len_s)), device=self.device)
        ...
        speech_frames = [i for i, e in enumerate(energy_label) if e == 0]

        # mask = Mask(self.device, coordinates, self.sample_rate).mask_2d_tau(self.max_difference_samples)
        ...
        x_frame = x[:, idx_in:idx_out]

        m_out_feature = feature.calculate_full(x_frame)

        # parameters = {'snr': -255,
        #               'rt_60': rt_60_desired,
        #               'signal_type': signal_type}

        return m_out_feature, desired_label
